...
$.fn.yi_scroll = function(options)
{
	...
	return this.each(function(){
		...
		var jqScrollBar= $("<div style='position:absolute;cursor:pointer;opacity:"+(opts.isBar?1:0)+";width:"+opts.barWidth+"px;top:"+opts.barOffTop+"px;right:"+opts.barOffRight+"px;border-radius: 5px;background: "+color.bar+";'></div>");
		...
		function fnContentResize()								/*容器内元素变动更新滚动*/
		{
			...
			jqScrollBox.show();
			iScrollBoxHeight = jqScrollBox.outerHeight()-50;
			iScrollBarHeight = Math.round(iRate*iScrollBoxHeight);
			jqScrollBar.css("height",iScrollBarHeight+50);
			iMinTop = elem.innerHeight() - elChild.outerHeight();
